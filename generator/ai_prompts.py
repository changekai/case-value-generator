...
def get_m1_prompt(industry: str, transformation_topic: str) -> str:
    """Step 1 — M1 全球趨勢洞察 Prompt"""
    return f"""針對「{industry}」行業中「{transformation_topic}」的AI轉型主題，
請從全球宏觀視角分析以下四個趨勢面向，
...
def get_m2_prompt(industry: str, transformation_topic: str) -> str:
    """Step 2 — M2 台灣在地市場分析 Prompt"""
    return f"""針對台灣「{industry}」行業的在地市場現況，
...
def get_c1_prompt(industry: str, transformation_topic: str) -> str:
    """Step 3 — C1 社會需求與社會價值 Prompt"""
    return f"""針對「{industry}」行業的「{transformation_topic}」AI應用，
請論述其社會需求程度與社會價值。
...
def get_c2_prompt(
    industry: str, transformation_topic: str, company_name: str
) -> str:
    """Step 4 — C2 潛在業者與競爭生態 Prompt"""
    return f"""針對台灣「{industry}」行業，
...
def get_who_prompt(
    ...
    return f"""業者「{company_name}」是一家台灣的「{industry}」業者，
